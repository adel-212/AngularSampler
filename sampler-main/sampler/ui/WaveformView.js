export class WaveformView {
  constructor(canvas, overlayPlayhead, overlayTrim) {
    this.canvas = canvas;
    this.playheadCanvas = overlayPlayhead;
    this.trimCanvas = overlayTrim;
  ...
  drawWave() {
    // Get dimensions from parent wrapper (like ex3)
    const wrap = this.canvas.parentElement;
    const w = wrap ? wrap.clientWidth : 500;
    const h = wrap ? wrap.clientHeight : 180;

    // Set all canvas dimensions to match wrapper
    this.canvas.width = w;
    this.canvas.height = h;
    this.playheadCanvas.width = w;
    this.playheadCanvas.height = h;
    this.trimCanvas.width = w;
    this.trimCanvas.height = h;
